...
(defn base-state
  ([] (base-state ::d.c.g/lifecycle.disconnected))
  ([lifecycle]
   {::d.c.g/halt-ch    (a/chan)
    ::d.c.g/websocket  (mock-websocket)
    ::d.c.g/control-ch (a/chan)
    ::d.c.g/output-ch  (a/chan 100)
    ::d.c.g/lifecycle  lifecycle}))

(defn bad-socket [state]
  (assoc state ::d.c.g/websocket (mock Websocket {:connect (throws Exception "Bad socket!")})))

(defn with-control-responses [{::d.c.g/keys [control-ch] :as state} & responses]
  (a/onto-chan! control-ch responses)
  state)

(defn with-resume-session-id [state]
  (assoc state ::d.c.g/resume-session-id 123123))

(defn with-simulated-websocket
  ([state]
   (assoc state ::d.c.g/websocket (mock-websocket-simulated)))
  ([state msg-ch]
   (let [sock (mock-websocket-simulated)]
     (inject sock msg-ch)
     (assoc state ::d.c.g/websocket sock))))

(deftest GatewayLifecycleTransitions

  (binding [log/*logger-factory* logi/disabled-logger-factory]
    (with-redefs [d.c.g/retry-delay-ms 0
                  d.c.g/timeout-ms     100]

      (testing "disconnected: bad initial connection -> disconnected state"
        (is (= ::d.c.g/lifecycle.disconnected
               (::d.c.g/lifecycle (a/<!! (gateway-lifecycle (-> (base-state)
                                                                bad-socket)))))))

      (testing "disconnected: ok initial connection -> connecting state"
        (is (= ::d.c.g/lifecycle.connecting
               (::d.c.g/lifecycle (a/<!! (gateway-lifecycle (base-state)))))))

      (testing "connecting: receives hello -> identifying state"
        (let [state (-> (base-state ::d.c.g/lifecycle.connecting)
                        (with-control-responses ws-hello-response))]
          (is (= ::d.c.g/lifecycle.identifying
                 (::d.c.g/lifecycle (a/<!! (gateway-lifecycle state)))))))
      ...
      (testing "identifying: receives ready -> connected state"
        (let [state (-> (base-state ::d.c.g/lifecycle.identifying)
                        (with-control-responses ws-ready-response))]
          (is (= ::d.c.g/lifecycle.connected
                 (::d.c.g/lifecycle (a/<!! (gateway-lifecycle state)))))))
      ...
      (testing "connected: control-ch closes -> disconnected state"
        (let [state      (-> (base-state ::d.c.g/lifecycle.connected)
                             (with-control-responses ws-reconnect-response))
              control-ch (::d.c.g/control-ch state)]
          (a/close! control-ch)
          (is (= ::d.c.g/lifecycle.disconnected
                 (::d.c.g/lifecycle
                   (a/<!!
      ...
      (testing "identifying: resumable, calls websocket/send-msg"
        (let [state     (-> (base-state ::d.c.g/lifecycle.identifying) with-resume-session-id)
              websocket (::d.c.g/websocket state)]
          (a/<!! (gateway-lifecycle state))
          (is (received? websocket ws/send-msg))))
      )))
...
(defn until-lifecycle [lifecycle]
  (fn [state]
    (when (= lifecycle (::d.c.g/lifecycle state))
      state)))
...
(deftest GatewayLifecycleWhole
  ...
  (binding [log/*logger-factory* logi/disabled-logger-factory]
    (with-redefs [d.c.g/retry-delay-ms 0
                  d.c.g/timeout-ms     1000]

      (testing "normal startup"
        (let [state (-> (base-state)
                        with-simulated-websocket
                        (execute-until #(= ::d.c.g/lifecycle.connected (::d.c.g/lifecycle %))))]
          (is (= ::d.c.g/lifecycle.connected (::d.c.g/lifecycle state))
              "Connected state is reached")))

      (testing "reconnect request"
        (let [state     (-> (base-state)
                            (with-simulated-websocket (adelay ws-reconnect-response 100))
                            (execute-until (-> (until-lifecycle ::d.c.g/lifecycle.connected)
                                               (times 2))))
              websocket (::d.c.g/websocket state)]
          (is (= ::d.c.g/lifecycle.connected (::d.c.g/lifecycle state))
...
(deftest HeartbeatRoutine
  (binding [log/*logger-factory* logi/disabled-logger-factory]
    (testing "heartbeat manual halt"
      (let [halt-ch (a/chan)
            ch      (d.c.g/begin-periodic-heartbeat (mock-websocket) 1000 halt-ch)]
        (a/close! halt-ch)
        (is (nil? (a/<!! ch)))))

    (testing "closes after interval"
      (let [ch (d.c.g/begin-periodic-heartbeat (mock-websocket) 10 (a/timeout 10000))]
        (is (nil? (a/<!! ch)))))

    (testing "sends requested heartbeat"
      (let [halt-ch   (a/timeout 100)
            websocket (mock-websocket {d.c.g/heartbeat-opcodes [ws-heartbeat-request]})]
        (a/<!! (d.c.g/begin-periodic-heartbeat websocket 1000 halt-ch))
        (is (= 2 (call-count websocket ws/send-msg)))))

    (testing "sends periodic heartbeats"
      (let [halt-ch   (a/timeout 100)
            websocket (mock-websocket {d.c.g/heartbeat-opcodes (deliver-interval
                                                                 10 (repeat 10 ws-heartbeat-ack))})]
        (a/<!! (d.c.g/begin-periodic-heartbeat websocket 10 halt-ch))
